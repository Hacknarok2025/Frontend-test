...
import { initPlatforms, initPlayer, initEnemy, checkGameOver } from './utils/gameManager';
...
const Level9 = () => {
  ...
  const drawNordicCorner = (p5: p5Types, x: number, y: number, size: number, position: number) => {
    p5.push();
    p5.translate(x, y);
  ...
  const drawGameOverScreen = (p5: p5Types) => {
    ...
    if (playerWon) {
      ...
      p5.text('YOU FINISHED THE GAME', p5.width/2, p5.height/2 - p5.height/8); // Increased spacing
      
      // Score display
      p5.textSize(p5.min(30, p5.width/21));
      p5.text(`You have defeated the ice giant!`, p5.width/2, p5.height/2); // Increased spacing
      p5.text(`Score: ${healthScore}/200`, p5.width/2, p5.height/2 + p5.height/8); // Increased spacing
      
      // Play again instruction
      p5.textSize(p5.min(20, p5.width/32));
      p5.text('Press SPACE to play again', p5.width/2, p5.height/2 + p5.height/4); // Increased spacing
      ...
      p5.fill(255); // White text
      p5.textSize(p5.min(30, p5.width/21));
      p5.text('The ice giant was too powerful!', p5.width/2, p5.height/2 - p5.height/10); // Increased spacing
      
      // Show player's final health score
      const healthScore = playerRef.current ? playerRef.current.health * 40 : 0;
      p5.text(`Final score: ${healthScore}/200`, p5.width/2, p5.height/2 + p5.height/10); // Increased spacing
      
      // Play again instruction
      p5.textSize(p5.min(20, p5.width/32));
      p5.text('Press SPACE to play again', p5.width/2, p5.height/2 + p5.height/5); // Increased spacing
    }
  ...
  const drawVictoryRunes = (p5: p5Types, x: number, y: number, frame: number) => {
    const runeCount = 8;
    const radius = 160;
  ...
  const drawParticles = (p5: p5Types, isVictory: boolean) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const x = p5.width / 2 + p5.cos(angle + frameCountRef.current * 0.01) * distFromCenter;
      const y = p5.height / 2 + p5.sin(angle + frameCountRef.current * 0.01) * distFromCenter;
  ...
  const drawGameplay = (p5: p5Types) => {
    ...
    for (const platform of platformsRef.current) {
      if (imagesRef.current.platform) {
        ...
        for (let i = 0; i < repeatCount; i++) {
          const drawWidth = Math.min(platformImg.width, platform.width - i * platformImg.width);
          p5.image(
            platformImg,
    ...
    p5.strokeWeight(2);
    p5.stroke(60, 40, 20);
    p5.rect(0, groundLevelRef.current, p5.width, p5.height - groundLevelRef.current);
    ...
    for (let i = 0; i < p5.width; i += 30) {
      p5.line(
        i, groundLevelRef.current + 5, 
        i + 15, groundLevelRef.current + 5
      );
    }
    
    for (let i = 15; i < p5.width; i += 30) {
      p5.line(
        i, groundLevelRef.current + 15, 
        i + 15, groundLevelRef.current + 15
      );
    }
    ...
    applyPhysics(playerRef.current, groundLevelRef.current, platformsRef.current, p5);
    if (wasAirborne && playerRef.current.velocityY === 0) {
      // Player just landed, create dust effect
      createDustEffect(p5, playerRef.current.x, playerRef.current.y + playerRef.current.height);
    }
    
    // Update enemy AI
    updateEnemyAI(p5, enemyRef.current, playerRef.current, frameCountRef.current, addAttack);
    
    // Apply physics to enemy
    applyPhysics(enemyRef.current, groundLevelRef.current, platformsRef.current, p5);
    
    // Update attacks with improved visuals
    updateAttacks(p5, attacksRef.current, playerRef.current, enemyRef.current, (isPlayerHit) => {
    ...
    drawAttacksWithEffects(p5, attacksRef.current, imagesRef.current.hammer, imagesRef.current.iceball);
    
    // Check game over conditions
    const gameOverResult = checkGameOver(playerRef.current.health, enemyRef.current.health);
  ...
  const createImpactEffect = (p5: p5Types, x: number, y: number, isLightning: boolean) => {
    for (let i = 0; i < 20; i++) {
