...
const Level7 = () => {
  ...
  const handleGameEnd = () => {
    ...
    // Calculate final score based on dodged fireballs
    const finalScore = Math.floor(
      scoreRef.current * (difficultyRef.current * 0.5)
    );
      ...
      if (backgroundImgRef.current && canvasSizeRef.current.width > 0) {
        ...
        const checkInterval = setInterval(() => {
          if (backgroundImgRef.current && !canvasReadyRef.current) {
            ...
            setCanvasReady(true);
            console.log(
              'Canvas resized with dimensions:',
              canvasSizeRef.current
            );
          }
        }, 100);
    ...
    const draw = (p5: p5Types) => {
      ...
      if (
        ...
        vikingXRef.current < p5.width - VIKING_WIDTH / 2
      ) {
        vikingXRef.current += vikingSpeed;
      }
      ...
      if (p5.millis() - lastFireballTimeRef.current > spawnInterval) {
        ...
        for (let i = 0; i < fireballCount; i++) {
          const newFireball = {
            ...
            y: -FIREBALL_HEIGHT / 2,
            speed:
              (3 + difficultyRef.current + p5.random(-1, 2)) *
              (p5.height / 360), // Scale speed based on canvas height
    ...
    return (
      <div
        className="viking-game-container"
        style={{
        ...
        <Modal3 open={isModalOpen} onClose={handleModalClose}>
          ...
          >
            ...
            <Button
              ...
              add={
                'text-white text-3xl hover:bg-white hover:text-black border-2 border-solid border-black'
              }
            >
              Try Again!
            </Button>
