...
const Level7 = () => {
  ...
  const draw = (p5: p5Types) => {
    ...
    if (!gameStarted && !gameOver) {
      ...
      p5.text(
        'You have 10 points, lose 1 for each hit',
        p5.width / 2,
        p5.height / 2 + p5.height / 7
      );
    ...
    if (gameOver) {
      ...
      p5.textSize(p5.min(20, p5.width / 32));
      p5.text(
        `Best score: ${Math.max(highScore, score)}`,
        p5.width / 2,
        p5.height / 2 + p5.height / 15
      );
      p5.text(
    ...
    if (p5.millis() - lastFireballTimeRef.current > spawnInterval) {
      ...
      for (let i = 0; i < fireballCount; i++) {
        const newFireball = {
          ...
          y: -FIREBALL_HEIGHT / 2,
          speed:
            (3 + difficultyRef.current + p5.random(-1, 2)) * (p5.height / 360), // Scale speed based on canvas height
