    ...
    return () => {
      if (p5Instance.current) {
        if (canvasParentRef.current) {
          const existingCanvases = canvasParentRef.current.getElementsByTagName('canvas');
  ...
  const checkWinCondition = () => {
    ...
    if (playerGridX === COLS - 3 && playerGridY === ROWS - 3) {
      ...
      if (finalTime > GRACE_PERIOD) {
        ...
        const finalScore = Math.max(0, INITIAL_SCORE - Math.floor(timeAfterGrace * SCORE_MULTIPLIER));
        setScore(finalScore);
      } else {
        setScore(INITIAL_SCORE);
      }
        ...
        >
          ...
          >
            ...
            {currentTime <= GRACE_PERIOD ? (
                ...
                <p className="text-xl">
                  (Score calculation: {INITIAL_SCORE} - ({currentTime - GRACE_PERIOD}s × {SCORE_MULTIPLIER}))
                </p>
            )}
          </motion.div>
        </Modal3>
