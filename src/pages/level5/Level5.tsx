...
const Level5 = () => {
    ...
    // Keep track of last moved rune positions for highlighting
    const [lastMovedPositions, setLastMovedPositions] = useState<{row1: number, col1: number, row2: number, col2: number} | null>(null);
    ...
    const updateRuneSelection = (row: number, col: number, isSelected: boolean) => {
        setBoard(prevBoard => {
    ...
    const swapRunes = (rune1: Rune, rune2: Rune) => {
        ...
        if (matches.length > 0) {
            ...
            // Update the lastMovedPositions state for reference
            setLastMovedPositions({
                row1: rune1.position.row, 
                col1: rune1.position.col,
                row2: rune2.position.row, 
                col2: rune2.position.col
            });
            ...
            setTimeout(() => {
                setBoard(prevBoard => {
                    ...
                    revertedBoard[rune1.position.row][rune1.position.col].isSelected = false;
                    revertedBoard[rune2.position.row][rune2.position.col].isSelected = false;
    ...
    const startGame = () => {
        ...
        setScore(0);
        setMoves(0);
        setTime(45); // Changed from 30 to 45 seconds
        setGameOver(false);
        setGameStarted(true);
        setGameWon(false);
    ...
    // Initialize game board on component mount, but don't start timer until instructions are dismissed
    useEffect(() => {
        setBoard(initializeBoard());
    }, []);
    
    // Calculate time display
    const timeDisplay = `${Math.floor(time / 60)}:${Math.floor(time % 60) < 10 ? '0' : ''}${time % 60}`;
        ...
        >
            <AnimatePresence>
                {showInstructions && (
                    ...
                    >
                        <motion.div
                            className="bg-black/80 p-8 rounded-lg shadow-xl max-w-2xl text-white border border-white"
                            initial={{ scale: 0.5, opacity: 0 }}
                            animate={{ scale: 1, opacity: 1 }}
                            exit={{ scale: 0.5, opacity: 0 }}
                            transition={{ duration: 0.5, type: "spring" }}
                        >
                            <h1 className="text-3xl font-bold mb-6 text-center" style={{ fontFamily: 'Norse, serif' }}>
                                Runes of the Future
                            </h1>
                            
                            <div className="space-y-4 mb-8">
                                <p className="text-lg">
                                    The Vanir gods have blessed you with ancient runes, each holding magical powers and glimpses into the future.
                                </p>
                                <p className="text-lg">
                                    <span className="font-bold">How to play:</span>
                                </p>
                                <ul className="list-disc pl-6 space-y-2">
                                    <li>Drag and drop runes to swap them with adjacent runes</li>
                                    <li>Match 3 or more identical runes in a row or column</li>
                                    <li>Each match reveals the power of the runes and gives you points</li>
                                    <li>Reach {targetScore} points within 60 seconds to unlock the wisdom of the Vanir</li>
                                </ul>
                                
                                <div className="grid grid-cols-3 gap-4 mt-4">
                                    {Object.entries(runeDefinitions).map(([type, { symbol, meaning }]) => (
                                        <div key={type} className="flex items-center gap-3 bg-gray-800/50 p-2 rounded">
                                            <span className="text-3xl" style={{ fontFamily: 'Norse, serif' }}>{symbol}</span>
                                            <div>
                            ...
                            <div className="flex justify-center">
                                <Button
                                    onClick={startGame}
                                    add="px-8 py-3 text-xl hover:bg-white hover:text-black border-2 border-solid border-white text-white"
                                >
                                    Start Game
                                </Button>
                            </div>
            ...
            <div className="bg-black/60 p-4 rounded-lg shadow-2xl w-full max-w-3xl mx-auto">
                <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
                    <h1 className="text-3xl font-bold text-white" style={{ fontFamily: 'Norse, serif' }}>
                        Runes of the Future
                    </h1>
                ...
                <div className="text-center mb-4 text-white text-sm">
                    <p>Match 3+ identical runes to reveal Vanir wisdom. Reach {targetScore} points to win!</p>
                </div>
                    ...
                    {board.flat().map(rune => (
                        <motion.div
                            ...
                            className={`aspect-square flex items-center justify-center cursor-grab active:cursor-grabbing rounded-sm ${
                                rune.isSelected ? 'bg-blue-600' : 
                                highlightedRunes.some(r => r.id === rune.id) ? 'bg-green-500' : 
                                'bg-gray-700'
                            } hover:bg-gray-600 transition-colors`}
                            whileHover={{ scale: 1.05 }}
                            animate={{ scale: highlightedRunes.some(r => r.id === rune.id) ? [1, 1.2, 1] : 1 }}
                            transition={{ duration: 0.2 }}
                            onMouseDown={() => handleDragStart(rune.position.row, rune.position.col)}
                            onMouseUp={() => handleDragEnd(rune.position.row, rune.position.col)}
                            onMouseEnter={() => {
                                if (dragStartPosition && !gameOver) {
                                    // If we're dragging, consider this a potential drop target
                                    const rowDiff = Math.abs(dragStartPosition.row - rune.position.row);
                                    const colDiff = Math.abs(dragStartPosition.col - rune.position.col);
                                    
                                    if ((rowDiff === 1 && colDiff === 0) || (rowDiff === 0 && colDiff === 1)) {
                                        updateRuneSelection(rune.position.row, rune.position.col, true);
                                    }
                                }
                            }}
                            onMouseLeave={() => {
                                if (dragStartPosition && !gameOver) {
                                    updateRuneSelection(rune.position.row, rune.position.col, false);
                                }
                            }}
                        >
                            <span className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-white select-none" style={{ fontFamily: 'Norse, serif' }}>{rune.symbol}</span>
                        </motion.div>
                    ))}
                </div>
            ...
            >
                <h1 className="text-3xl mb-6 font-bold" style={{ fontFamily: 'Norse, serif' }}>
